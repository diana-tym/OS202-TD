...
deb = time()
for y in range(proc_lignes_n * rank, proc_lignes_n * (rank + 1)):
    for x in range(width):
        c = complex(-2. + scaleX*x, -1.125 + scaleY * y)
        local_convergence[y - proc_lignes_n * rank, x] = mandelbrot_set.convergence(c, smooth=True)
fin = time()
...
comm.Gather(local_convergence, convergence)
